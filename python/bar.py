...
Each of N friends chooses a number from his list and reports it to the game administrator. Then the game administrator
sorts the reported numbers and shouts the K-th largest number.
...
In the first line there are two numbers N and K. In each of next N lines there is an integer a_i followed by a_i
integers, ith line describes the list for ith person.
...
, {8 1 6} for second and {7 4 9} for the third. In this case all the numbers in {4, 5, 6, 7, 8, 9} have a chance to be
the third biggest selected number.
...
def problem(stdin):
    lines = [map(int, x.split()) for x in stdin.split('\n')]
    t = lines.pop(0)[0]
    for i in range(t):
        n, k = lines.pop(0)
        numbers = []
...
def solve(k, players_numbers):
    """
    k ... the k-th SMALLEST number
    """
    pairs = []
    for pidx in range(len(players_numbers)):
        pairs += ((x, pidx) for x in players_numbers[pidx])
    pairs.sort(key=lambda x: x[0])

    players_with_smaller_numbers = set()
    lower_idx = 0
    for p in pairs:
        players_with_smaller_numbers.add(p[1])
        if len(players_with_smaller_numbers) == k:
            break
        lower_idx += 1

    players_with_greater_numbers = set()
    upper_idx = len(pairs) - 1
    for p in reversed(pairs):
        players_with_greater_numbers.add(p[1])
        if len(players_with_greater_numbers) == len(players_numbers) - k + 1:
            break
        upper_idx -= 1
